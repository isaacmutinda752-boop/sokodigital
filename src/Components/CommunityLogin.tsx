...
import { FaEnvelope, FaLock, FaArrowRight, FaUsers, FaComments, FaHandshake, FaLightbulb } from 'react-icons/fa6';
...
const CommunityLogin = () => {
    return (
        <section className="min-h-screen bg-slate-950 py-24 px-6 md:px-12 relative overflow-hidden flex items-center justify-center">
            ...
            >
                {/* Left Side: Community Welcome Back */}
                <div className="space-y-10">
                    <div className="space-y-6">
                        ...
                        <h2 className="text-4xl md:text-6xl font-black text-white leading-[1.1]">
                            Welcome Back to the <span className="text-purple-400">Circle.</span>
                        </h2>
                        <p className="text-slate-400 text-lg leading-relaxed max-w-lg">
                            Log in to rejoin the conversation, share your latest builds, and collaborate with the brightest minds in Kenya's digital space.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        {[
                            { title: "Discussion Feed", desc: "Catch up on the latest tech trends.", icon: <FaComments /> },
                            { title: "Member Perks", desc: "Access exclusive community resources.", icon: <FaHandshake /> },
                            { title: "Showcase", desc: "Share your projects and get feedback.", icon: <FaLightbulb /> },
                            { title: "Networking", desc: "Directly message other builders.", icon: <FaUsers /> }
                        ].map((item, i) => (
                ...
                <div className="bg-slate-900/40 backdrop-blur-xl border border-white/10 p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-purple-500/5 relative">
                    <div className="mb-10 text-center lg:text-left">
                        <h3 className="text-2xl font-black text-white">Login to Community</h3>
                        <p className="text-slate-500 text-sm mt-2">Enter your credentials to enter the hub.</p>
                    </div>
                    ...
                    <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <a href="#" className="text-purple-400 text-[10px] font-bold hover:underline">Forgot?</a>
                            </div>
                            <div className="relative group">
                        ...
                        <button className="w-full bg-purple-600 hover:bg-purple-700 text-white font-black py-5 rounded-2xl shadow-xl shadow-purple-600/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3 text-lg mt-4">
                            Enter the Hub <FaArrowRight />
                        </button>

                        <div className="text-center pt-4 border-t border-white/5">
                            <p className="text-slate-500 text-sm">
                                Not a member yet?{" "}
                                <Link to="/JoinTheCommunityAccount" className="text-purple-400 font-bold hover:underline">Register here</Link>
                            </p>
                        </div>
                    </form>
                </div>
